...
export default function AgentActivityCenterPage() {
  const agentCards = useIARStore((s) => s.agentCards);
  const a2aMessages = useIARStore((s) => s.a2aMessages);
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      {activeAgents.length > 0 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="mt-6 flex items-center gap-2 rounded-xl border border-iar-teal/30 bg-iar-teal/5 px-4 py-3"
        >
          <Radio className="h-4 w-4 animate-pulse text-iar-teal" />
