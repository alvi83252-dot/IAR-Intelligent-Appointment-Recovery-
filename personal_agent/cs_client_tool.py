"""Tool that lets the personal agent talk to the practice front desk (CS) agent
over A2A, propagating the current session's contextId so both agents (and the
env) share one conversation identity."""
...
async def ask_customer_service(message: str, tool_context: ToolContext) -> str:
    ...
    The conversation with front desk persists for this whole session, so you can
    ask follow-up questions and they will remember the context.
    """
    outgoing = Message(
    ...
    return reply or "[no response from front desk]"
